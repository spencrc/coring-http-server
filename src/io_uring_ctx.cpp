...
io_uring_ctx::io_uring_ctx(unsigned int queue_depth) {
	...
	if (ret < 0)
		throw std::system_error(
			...
			"Failed to initialize io_uring submissions & completion queue");
}
...
io_uring_ctx::io_uring_ctx(unsigned int queue_depth, io_uring_params *params) {
	...
	if (ret < 0)
		throw std::system_error(-ret, std::generic_category(),
								"Failed to initialize io_uring submissions & "
								"completion queue with params");
}
...
unsigned int io_uring_ctx::peek_batch_cqe(io_uring_cqe **cqe_ptrs,
										  unsigned int count) {
...
void io_uring_ctx::accept_direct(int sockfd, void *data,
...
void io_uring_ctx::read(int clientfd, std::array<char, BUFFER_LEN> *buf,
						void *data, unsigned int sqe_flags) {
	io_uring_sqe *sqe = io_uring_get_sqe(&ring);
...
void io_uring_ctx::write(int clientfd, std::array<char, BUFFER_LEN> *buf,
						 unsigned int write_len, void *data,
						 unsigned int sqe_flags) {
	io_uring_sqe *sqe = io_uring_get_sqe(&ring);
...
int io_uring_ctx::register_files_sparse(unsigned int nr_files) {
	...
	if (ret < 0)
		throw std::system_error(-ret, std::generic_category(),
								"Failed to sparse register files");
	fixed_files = true;
	return ret;
}

int io_uring_ctx::register_files_update(unsigned int offset,
